...
<div class="container" style="text-align: center; opacity: 0.55; background-color: #708f75; margin-top: 50px; margin-left: 80px; margin-right: 30px; padding-top: 30px; padding-bottom: 30px;">
    <?php
        ...
        $query = "SELECT MAX(cantitate), MIN(cantitate)
        FROM livrari
        WHERE idc = 'C12';";
        ...
        echo "<h4>Să se găsească pentru componenta cu codul ’C12’ cantitatea minimă și cantitatea maximă livrată.</h4>";
        echo ' <Table class="table">
        <tr>
            <th>#</th>
            <th>Cantitatea maxima</th>
            <th>Cantitatea minima</th>
        </tr>'; 
        ...
        for ($i=0; $i < $rows; $i++)
        {
            ...
            echo '<td>'.stripslashes($row['MAX(cantitate)']).'</td>';
            echo '<td>'.stripslashes($row['MIN(cantitate)']).'</td>';
            echo '</tr>';
        }
