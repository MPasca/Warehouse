...
<div class="container" style="text-align: center; opacity: 0.55; background-color: #708f75; margin-top: 50px; margin-left: 80px; margin-right: 30px; padding-top: 30px; padding-bottom: 30px;">
	...
	<?php
		...
		{
			die('Connection errror:'. $dsn->connect_error);
		}
		
		echo '<h4>Să se găsească detaliile componentelor cu masa între 100 și 500 și care sunt în ‘Cluj-Napoca’.</h4>';
		$query = "SELECT * FROM componente WHERE masa BETWEEN 100 AND 500 AND oras = 'Cluj - Napoca';";
		...
		for ($i=0; $i < $rows; $i++)
		{
			...
			echo '<td>'.htmlspecialchars(stripslashes($row['idc'])).'</td>';
			echo '<td>'.stripslashes($row['numec']).'</td>';
			echo '<td>'.stripslashes($row['culoare']).'</td>';
			echo '<td>'.stripslashes($row['masa']).'</td>';
